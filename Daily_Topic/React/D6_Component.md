...
1. 组件与与常规的类和函数有什么区别？
   ...
   > 组件本质上就是函数和类，但是组件又承载了渲染视图的的 UI 和更新视图的 setState, useState 等方法。React 在底层逻辑上会像正常实例化类和正常执行函数那样处理的组件 React 在底层逻辑上会像正常实例化类和正常执行函数那样处理的组件函数和类所具有的特性组件也是有的
...
react 处理 Component 的逻辑是在类组件执行构造函数的时候会在实例上绑定 props 和 context,初始化置空 ref 属性。在原型链上绑定 setState 和 forceUpdate 方法。
